...
async function buscarMenus(){
    //aqui hacemos el llamado de nuestra promesa
    const menu = await getMenus();
    const cont = document.getElementById('container');
    const title = document.createElement('h2');
    const tam = menus.length;

    cont.innerHTML = "";
    title.textContent = "Menu del dia";
    ...
    for(let i = 0; i<tam; i++){
        ...
        for (let propiedad in menu){
            tarjet.innerHTML += `${menu[propiedad]}<br>`;
        }
        cont.appendChild(tarjet);
    }
}

//ejecutamos la funcion asincronica
buscarMenus();